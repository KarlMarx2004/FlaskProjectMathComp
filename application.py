...
def confirm(contestnumber):
    ...
    u = Contest1Sub.query.filter_by(username=username).first()
    if u:
        return render_template("error.html", message="You have already taken this contest! Go back home")
...
def write():
    if request.method == "POST":
        ...
        username = session["username"]
        guide = Guide(title=title, subheading=subheading, article=article, user=username)
        db.session.add(guide)
        db.session.commit()
        return render_template("error.html", message="Success! You published a guide.")
    if "logged_in" not in session or session.get("logged_in") == False:
        return redirect('/login')
...
@app.route("/guide/<guidenumber>")
def guide(guidenumber):
    guide = Guide.query.get(guidenumber)
    return render_template("individualguide.html", guide=guide)
